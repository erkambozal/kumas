  ...
  <div class="container-scroller">
    ...
    <nav class="navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
      <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-center">
        <a class="navbar-brand brand-logo mr-5" href="/anasayfa"><img src="{{ asset('img/tanerk.png') }}" class="mr-2" alt="logo"/></a>
        <a class="navbar-brand brand-logo-mini" href="/anasayfa"><img src="{{ asset('img/tanerkmini.png') }}" alt="logo"/></a>
      </div>
          ...
          <li class="nav-item dropdown">
            <a class="nav-link" id="profileDropdown" href="#" data-toggle="dropdown" aria-expanded="false">
              <div class="navbar-profile">
                <img class="img-xs rounded-circle" src="{{ asset('img/tanerkmini.png') }}" alt="profile image">
            ...
            <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  ...
                  <form action="{{route('duzenlePost',$product->id)}}" enctype="multipart/form-data" method="POST">
                    @csrf
                    @php
                        $selectedCategories = json_decode($product->categories, true);
                        $selectedImages = json_decode($product->images, true);
                        @endphp
                      ...
                      <div class="kategoriler">
                          ...
                          @foreach($categories as $category)
                              @if ($category['cat_ust'] == '1')
                                      <input type="checkbox" id="categories" name="categories[]" value="{{$category}}" {{ in_array($category['id'], array_column($selectedCategories, 'id')) ? 'checked' : '' }}> {{$category['name']}}<br>
                              @endif
                          @endforeach
                    ...
                    <div class="form-group">
                      ...
                      <div class="form-group">
                      <label for="listeleme">Status</label>
                      <select class="form-control" id="status" name="status">
                          <option value="0" {{ old('status', $product->status) == 0 ? 'selected' : '' }}>0</option>
                          <option value="1" {{ old('status', $product->status) == 1 ? 'selected' : '' }}>1</option>
                      </select>
                  </div>
                  <div class="form-group">
                    ...
                    @if (!empty($selectedImages))
                        <div>
                            ...
                            @foreach($selectedImages as $image)
                                <img src="{{ asset($image) }}" alt="Product Image" style="max-width: 200px; margin-right: 10px;">
                            @endforeach
                        </div>
                    @endif
                </div>
                    ...
                    <div class="form-group">
                      ...
                      <textarea class="form-control" name="content" rows="4">{{ old('content', $product->content) }}</textarea>
                  </div>
